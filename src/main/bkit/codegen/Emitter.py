...
from CodeGenError import * #NEWLY ADDED
...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is cgen.IntType:
            ...
            if not inType.dimen: return "["+ self.getJVMType(inType.eleType) #modified
            elif len(inType.dimen) == 0: return "["+ self.getJVMType(inType.eleType) #modified
            return "".join(["[" for _ in inType.dimen]) + self.getJVMType(inType.eleType) #modified
        elif typeIn is cgen.MType:
            return "(" + "".join(list(map(lambda x: self.getJVMType(x), inType.partype))) + ")" + self.getJVMType(inType.rettype)
        elif typeIn is cgen.ClassType:
            return "L" + inType.cname + ";"

    def getFullType(self, inType): #modified
        typeIn = type(inType)
        if typeIn is cgen.IntType:
            return "int"
        elif typeIn is cgen.FloatType:
    ...
    def emitPUSHICONST(self, in_, frame):
        ...
        if type(in_) is int:
            ...
            else: return self.jvm.emitLDC(str(i)) #NEWLY ADDED
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        if type(typ) is cgen.IntType: #redundant
            return self.emitPUSHICONST(int(in_), frame)
        if type(typ) is cgen.BoolType: #redundant
            return self.emitPUSHICONST(in_, frame)
        elif type(typ) is cgen.FloatType: #added for fun
            return self.emitPUSHFCONST(in_, frame)
        elif type(typ) is cgen.StringType:
            frame.push()
            return self.jvm.emitLDC(in_) #Might need to add double quotes
        else:
            #print(type(typ))
    ...
    def emitPUSHARRAY_SHALLOW(self, typ, frame):
        if type(typ) is cgen.ArrayType:
            if len(typ.dimen) > 1: #multidimension array
                frame.pop()
                frame.push()
                childType = cgen.ArrayType(typ.eleType, typ.dimen[1:])
                return self.jvm.emitANEWARRAY(self.getJVMType(childType))
            elif type(typ.eleType) is cgen.StringType:
                frame.pop()
                frame.push()
                return self.jvm.emitANEWARRAY(self.getFullType(typ.eleType))
            else: #Int or bool or float
                frame.pop()
                frame.push()
                return self.jvm.emitNEWARRAY(self.getFullType(typ.eleType))
        else:
            raise IllegalOperandException(in_) #not sure
        
    def emitPUSHARRAY_DEEP(self, typ, frame):
        if type(typ) is cgen.ArrayType: #different from shallow verion
            if len(typ.dimen) > 1: #multidimension array
                [frame.pop() for _ in typ.dimen]
                frame.push()
                return self.jvm.emitMULTIANEWARRAY(self.getJVMType(typ), str(len(typ.dimen)))
            elif type(typ.eleType) is cgen.StringType:
                frame.pop()
                frame.push()
                return self.jvm.emitANEWARRAY(self.getFullType(typ.eleType))
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isFinal, value=None):
        ...
        return self.jvm.emitSTATICFIELD(lexeme, self.getJVMType(in_), isFinal) #modified 
    ...
    *   generate ior
    ...
    def emitREFOP(self, op, in_, frame): #assembly programming is painfully slow...
        #op: String
        #in_: Type
        #frame: Frame
        #..., value1, value2 -> ..., result
        ...
        #stack x, y <- top
        frame.pop()     
        result.append(self.jvm.emitFCMPL()) #stack - 1 # r=1: x > y; r=0: x == y; r=-1: x < y             
        
        frame.pop()
        #stack - 1
        if op == ">.": #branch if x <= y --> r <= 0
            result.append(self.jvm.emitIFLE(labelF))
        elif op == ">=.": #branch if x < y --> r < 0
        ...
        result.append(self.emitPUSHCONST("1", cgen.IntType(), frame)) #stack + 1
        frame.pop() #stack - 1
        result.append(self.emitGOTO(labelO, frame))
        result.append(self.emitLABEL(labelF, frame))
        result.append(self.emitPUSHCONST("0", cgen.IntType(), frame)) #stack + 1
        result.append(self.emitLABEL(labelO, frame))
        return ''.join(result)
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def getConst(self, ast): #??? what is this used for
        #ast: Literal
    ...
    *   @param label the label where the execution continues if the value on top of stack is true.
    ...
    def emitIFTRUE(self, label, frame):
        ...
        frame.pop()
        #return self.jvm.emitIFGT(label) #OLD
    ...
    *   @param label the label where the execution continues if the value on top of stack is false.
    ...
    def emitIFFALSE(self, label, frame):
        ...
        frame.pop()
        #return self.jvm.emitIFLE(label)OLD
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame): #MODIFIED
        #in_: Type
        #frame: Frame
